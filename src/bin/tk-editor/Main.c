...
int main(int argc, char **argv_pp) 
{
    ...
    tk_api_Interface *EditorInterface_p = tk_api_createEditorInterface();

    if (!EditorInterface_p) {
        puts("EditorInterface creation failed.");
        return 1;
    }

    Session_p = tk_api_openSession(NULL, EditorInterface_p, 1);
