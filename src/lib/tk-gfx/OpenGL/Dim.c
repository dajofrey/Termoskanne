...
static TK_API_RESULT tk_gfx_initOpenGLDimProgram(
    tk_gfx_OpenGLDim *Dim_p, nh_gfx_OpenGLCommandBuffer *CommandBuffer_p)
{
...
static TK_API_RESULT tk_gfx_initOpenGLDimVertices(
    tk_gfx_OpenGLDim *Dim_p, nh_gfx_OpenGLCommandBuffer *CommandBuffer_p)
{
...
static TK_API_RESULT tk_gfx_updateOpenGLDimVertices(
    tk_gfx_OpenGLDim *Dim_p, nh_gfx_OpenGLCommandBuffer *CommandBuffer_p, nh_core_Array *Vertices_p)
{
    nh_gfx_addOpenGLCommand(CommandBuffer_p, "glBindVertexArray", Dim_p->VertexArray_p);
    ...
    void *state_p, void *data_p)
{
    ...
    tk_gfx_Dim *Dim_p = data_p;

    if (Dim_p->Action.init) {
        tk_gfx_initOpenGLDimProgram(&Dim_p->OpenGL, 
            State_p->Viewport_p->OpenGL.CommandBuffer_p);
        tk_gfx_initOpenGLDimVertices(&Dim_p->OpenGL, 
            State_p->Viewport_p->OpenGL.CommandBuffer_p);
        Dim_p->Action.init = false;
    }

    tk_gfx_updateOpenGLDimVertices(&Dim_p->OpenGL,
...
{
    memset(Dim_p, 0, sizeof(tk_gfx_OpenGLDim));
