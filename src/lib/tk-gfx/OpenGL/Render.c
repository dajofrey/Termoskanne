...
static TK_API_RESULT tk_gfx_drawOpenGLDim(
    tk_gfx_Graphics *Graphics_p, tk_gfx_Grid *Grid_p)
{
...
static TK_API_RESULT tk_gfx_drawOpenGLBackground(
    tk_gfx_GraphicsState *State_p, tk_gfx_GraphicsData *Data_p)
{
    ...
    for (int i = 0, offset = 0; i < Data_p->Background.Ranges.length; ++i) {
        tk_gfx_AttributeRange *Range_p = ((tk_gfx_AttributeRange*)Data_p->Background.Ranges.p)+i;
...
static TK_API_RESULT tk_gfx_drawOpenGLForeground(
    tk_gfx_GraphicsState *State_p, tk_gfx_GraphicsData *Data_p)
{
    nh_gfx_OpenGLCommandBuffer *CommandBuffer_p = State_p->Viewport_p->OpenGL.CommandBuffer_p;
    ...
    for (int i = 0, offset = 0; i < Data_p->Foreground.Ranges.length; ++i) {
        tk_gfx_AttributeRange *Range_p = ((tk_gfx_AttributeRange*)Data_p->Foreground.Ranges.p)+i;
    ...
    for (int i = 0, offset = 0; i < Data_p->Foreground.Ranges2.length; ++i) {
        tk_gfx_AttributeRange *Range_p = ((tk_gfx_AttributeRange*)Data_p->Foreground.Ranges2.p)+i;
        nh_gfx_addOpenGLCommand(CommandBuffer_p, "glDrawElements", 
            nh_gfx_glenum(NULL, GL_TRIANGLES), nh_gfx_glsizei(NULL, Range_p->indices), 
            nh_gfx_glenum(NULL, GL_UNSIGNED_INT),
            nh_gfx_glpointer(NULL, (void*)(sizeof(uint32_t)*offset)));
...
static TK_API_RESULT tk_gfx_drawOpenGLElevatedBackground(
    tk_gfx_Graphics *Graphics_p, tk_gfx_GraphicsData *BackdropData_p, tk_gfx_GraphicsData *MainData_p,
    tk_gfx_Grid *BackdropGrid_p, tk_gfx_Grid *Grid_p)
{
    for (int i = 0; i < Graphics_p->Boxes.Data.length; ++i) {
        ...
        tk_gfx_drawOpenGLBackground(&Graphics_p->State, BackdropData_p);
        tk_gfx_drawOpenGLDim(Graphics_p, Grid_p);
    }

    return TK_API_SUCCESS;
}

static TK_API_RESULT tk_gfx_drawOpenGLInactiveCursor(
    tk_gfx_Graphics *Graphics_p, tk_gfx_Grid *Grid_p)
{
    nh_gfx_OpenGLCommandBuffer *CommandBuffer_p = Graphics_p->State.Viewport_p->OpenGL.CommandBuffer_p;
...
// render main grid
    ...
    TK_GFX_CHECK(tk_gfx_updateOpenGLDim(&Graphics_p->State, &Graphics_p->Dim))
 
    if (Config_p->style > 0) {
        TK_GFX_CHECK(tk_gfx_drawOpenGLDim(Graphics_p, Grid_p))
    }
    ...
    TK_GFX_CHECK(tk_gfx_updateOpenGLForeground(Config_p, &Graphics_p->State, &Graphics_p->MainData))
    TK_GFX_CHECK(tk_gfx_drawOpenGLForeground(&Graphics_p->State, &Graphics_p->MainData))

// draw elevated grid

    TK_GFX_CHECK(tk_gfx_drawOpenGLElevatedBackground(Graphics_p, &Graphics_p->BackdropData, &Graphics_p->MainData, BackdropGrid_p, Grid_p))
    ...
    TK_GFX_CHECK(tk_gfx_updateOpenGLForeground(Config_p, &Graphics_p->State, &Graphics_p->ElevatedData))
    TK_GFX_CHECK(tk_gfx_drawOpenGLForeground(&Graphics_p->State, &Graphics_p->ElevatedData))
