...
static TK_API_RESULT tk_gfx_getForegroundVerticesDefault(
    tk_gfx_GraphicsState *State_p, tk_gfx_Grid *Grid_p, tk_core_Glyph *G_p, int col, 
    int row, float depth, float vertices_p[20])
{
    ...
    if (!Glyph_p) {
        ...
        if (glyphs != 1) {return TK_API_ERROR_BAD_STATE;}
        ...
        TK_GFX_CHECK_MEM(Glyph_p)
        char *codepoint_p = nh_core_allocate(sizeof(char)*4);
        TK_GFX_CHECK_MEM(codepoint_p)
...
static TK_API_RESULT tk_gfx_getForegroundVerticesForLineGraphics(
    tk_gfx_GraphicsState *State_p, tk_gfx_Grid *Grid_p, NH_API_UTF32 codepoint, int col,
    ...
    int row, float *vertices_p, int fontSize)
{
    ...
    TK_GFX_CHECK(tk_gfx_getForegroundVerticesForLineGraphics(
    ...
    int row, float *vertices_p, int fontSize)
{
    ...
    if (Glyph_p->mark & TK_CORE_MARK_LINE_GRAPHICS) {
        ...
        TK_GFX_CHECK(tk_gfx_getForegroundVerticesDefault(
