...
TK_API_RESULT tk_gfx_initGrid(
    tk_gfx_Grid *Grid_p)
{
    memset(Grid_p, 0, sizeof(tk_gfx_Grid));
    ...
    tk_gfx_Grid *Grid_p)
{
    ...
    tk_gfx_initGrid(Grid_p);

    return TK_API_SUCCESS;
}

static TK_API_RESULT tk_gfx_initTile(
    tk_gfx_Grid *Grid_p, tk_gfx_Tile *Tile_p, int row, int col)
{
    memset(Tile_p, 0, sizeof(tk_gfx_Tile));
...
static void tk_gfx_getTileRowAndColumn(
    tk_gfx_Grid *Grid_p, tk_gfx_Tile *Tile_p, int *row_p, int *col_p)
{
    bool done = false;
    ...
    tk_gfx_Grid *Grid_p, int row, int col)
{
    ...
    if (!Grid_p->Rows.pp[row]) {
        ...
        TK_GFX_CHECK_MEM_2(NULL, Grid_p->Rows.pp[row])
    ...
    if (!Cols_p->pp[col]) {
        Cols_p->pp[col] = nh_core_allocate(sizeof(tk_gfx_Tile));
        TK_GFX_CHECK_MEM_2(NULL, Cols_p->pp[col])
        TK_GFX_CHECK_2(NULL, tk_gfx_initTile(Grid_p, Cols_p->pp[col], row, col))
    }
...
static TK_API_RESULT tk_gfx_updateCursorTile(
    tk_gfx_Grid *Grid_p, tk_gfx_GraphicsState *State_p, tk_gfx_TileUpdate *Update_p)
{
    ...
    tk_gfx_Tile *Tile_p = ((nh_core_List*)Grid_p->Rows.pp[Update_p->row])->pp[Update_p->col];
    tk_gfx_Tile *OldCursor_p = Grid_p->Cursor_p;
    ...
    int row, col = 0;
    tk_gfx_getTileRowAndColumn(Grid_p, OldCursor_p, &row, &col);
...
static TK_API_RESULT tk_gfx_updateTileVertices(
    tk_core_Glyph *Glyph_p, tk_gfx_GraphicsState *State_p, tk_gfx_Grid *Grid_p, int col,
    int row, tk_gfx_Tile *Tile_p, int target, int fontSize)
{
    switch (target)
    {
        case 2 :
            TK_GFX_CHECK(tk_gfx_getOverlayVertices(
                State_p, Grid_p, Glyph_p, col, row, Tile_p->Overlay.vertices_p, fontSize
            ))
            break;
        case 1 :
            TK_GFX_CHECK(tk_gfx_getForegroundVertices(
    ...
    tk_gfx_Grid *Grid_p, void *state_p, tk_gfx_TileUpdate *Update_p, bool *update_p, int fontSize)
{
    ...
    if (Update_p->cursor) {
        ...
        return tk_gfx_updateCursorTile(Grid_p, state_p, Update_p);
    }
    ...
    {
        TK_GFX_CHECK(tk_gfx_updateTileVertices(
    ...
    // Compare attributes.
    if (tk_gfx_compareForegroundAttributes(&Tile_p->Glyph, &Update_p->Glyph)
    ||  tk_gfx_compareBackgroundAttributes(&Tile_p->Glyph, &Update_p->Glyph))
    {
        Tile_p->dirty = true;
    }
    ...
    void *state_p, nh_gfx_Text *Text_p)
{
    ...
    // Free data.
    tk_gfx_freeGrid(BackdropGrid_p);
...
TK_API_RESULT tk_gfx_updateGrid(
    tk_gfx_Config *Config_p, tk_core_Config *CoreConfig_p, tk_gfx_Grid *Grid_p,
    void *state_p, nh_gfx_Text *Text_p)
{
    tk_gfx_GraphicsState *State_p = state_p;
    ...
    Grid_p->Updates_pp = (tk_gfx_TileUpdate**)nh_core_allocate(sizeof(tk_gfx_TileUpdate*) * Grid_p->rows);
    TK_GFX_CHECK_MEM(Grid_p->Updates_pp)

    Grid_p->updates_pp = (bool**)nh_core_allocate(sizeof(bool*) * Grid_p->rows);
    TK_GFX_CHECK_MEM(Grid_p->updates_pp)

    for (int row = 0; row < Grid_p->rows; ++row) {
        Grid_p->Updates_pp[row] = (tk_gfx_TileUpdate*)nh_core_allocate(sizeof(tk_gfx_TileUpdate) * Grid_p->cols);
        TK_GFX_CHECK_MEM(Grid_p->Updates_pp[row])
