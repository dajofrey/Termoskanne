...
static tk_core_Color tk_gfx_colorFromRGB(
    int r, int g, int b)
{
    tk_core_Color Color;
...
static tk_core_Color tk_gfx_blend(
...
static tk_core_Color tk_gfx_getStrobeColor(
...
static tk_core_Color tk_gfx_getWaveColor(
...
static tk_core_Color tk_gfx_getRainbowColor(
...
static tk_core_Color tk_gfx_getBubbleColor(
    int col, int row, float time) 
{
...
static tk_core_Color tk_gfx_getPastelCheckerColor(
    int col, int row, tk_core_Color base)
{
    int alt = (col / 2 + row / 2) % 2;
    if (alt)
        return tk_gfx_colorFromRGB(255, 235, 205); // Light peach
    else
        return tk_gfx_colorFromRGB(110, 145, 155); // Pale blue
}

static tk_core_Color tk_gfx_getOceanColor(
    ...
    int col, int row, int total_cols, int total_rows, tk_core_Color base)
{
    ...
    tk_core_Color sunGlow = tk_gfx_colorFromRGB(255, 204, 92);   // Yellow/orange near sun
    tk_core_Color horizonPink = tk_gfx_colorFromRGB(255, 105, 180);  // Pink
    tk_core_Color twilight = tk_gfx_colorFromRGB(128, 0, 128);    // Purple
    tk_core_Color nightSky = tk_gfx_colorFromRGB(25, 25, 112);    // Midnight blue
    tk_core_Color lowBlend = tk_gfx_blend(sunGlow, horizonPink, t);
    tk_core_Color highBlend = tk_gfx_blend(twilight, nightSky, t);
    tk_core_Color result = tk_gfx_blend(lowBlend, highBlend, t * 0.8f + 0.2f);
    return result;
}

static tk_core_Color tk_gfx_getAccentColor(
    tk_gfx_Config *Config_p, int col, int row, int total_cols, int total_rows)
{
    ...
    switch (Config_p->style) {
        case 1 : return tk_gfx_getWaveColor(col, row, time_now, total_cols, total_rows, base);
        case 2 : return tk_gfx_getStrobeColor(col, row, time_now, base);
        case 3 : return tk_gfx_getRainbowColor(col, row, time_now);
        case 4 : return tk_gfx_getBubbleColor(col, row, time_now);
        case 5 : return tk_gfx_getPastelCheckerColor(col, row, base);
        case 6 : return tk_gfx_getOceanColor(col, row, total_cols, total_rows, base);
        case 7 : return tk_gfx_getSunsetColor(col, row, total_cols, total_rows, base);
    }
    return base;
}

static inline tk_core_Color tk_gfx_getGlyphColor2(
    tk_gfx_Config *Config_p, tk_gfx_GraphicsState *State_p, tk_core_Glyph *Glyph_p, unsigned int target,
...
{
    tk_core_Color Color = tk_gfx_getGlyphColor2(Config_p, State_p, Glyph_p, target, col, row, Grid_p);
    Color.a = 1.0f;
...
tk_core_Color tk_gfx_getOverlayColor(
    ...
    int col, int row, tk_gfx_Grid *Grid_p)
{
    tk_core_Color Color = tk_gfx_getGlyphColor2(Config_p, State_p, Glyph_p, 2, col, row, Grid_p);
    Color.a = 1.0f;
