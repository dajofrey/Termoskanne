...
static TK_API_RESULT tk_core_setMessage(
    tk_core_Topbar *Topbar_p, NH_API_UTF32 *message_p, int length)
{
    ...
    if (Topbar_p) {
        ...
        Message_p = &TK_CORE_MACRO_TAB(((tk_core_Session*)nh_core_getWorkloadArg())->Window_p->Tile_p)->Topbar.Message;
    }
...
TK_API_RESULT tk_core_clearMessage(
...
TK_API_RESULT tk_core_setDefaultMessage(
...
TK_API_RESULT tk_core_setCustomMessage(
...
TK_API_RESULT tk_core_setCustomSuffixMessage(
...
TK_API_RESULT tk_core_setBinaryQueryMessage(
    tk_core_Topbar *Topbar_p, NH_API_UTF32 *query_p, int length, void *args_p, 
    TK_API_RESULT (*callback_f)(nh_api_KeyboardEvent Event, bool *continue_p))
{
//    tk_core_TopbarMessage *Message_p = &((tk_core_MacroTile*)((tk_core_Session*)nh_core_getWorkloadArg())->Window_p->Tile_p->p)->Topbar.Message;
//    if (Message_p->block) {TK_CORE_DIAGNOSTIC_END(TK_API_ERROR_BAD_STATE)}
//
//    int yesNoLength;
