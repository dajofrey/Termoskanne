...
TK_API_RESULT tk_core_getTopbarCursor(
...
static TK_API_RESULT tk_core_appendToCommand(
...
static TK_API_RESULT tk_core_resetTopbar(
    tk_core_Topbar *Topbar_p)
{
    Topbar_p->state = -1;
    ...
    tk_core_Topbar *Topbar_p)
{
    if (!Topbar_p->hasFocus) {
        ...
        if (((tk_core_Session*)nh_core_getWorkloadArg())->Prototypes.size < 2) {
...
//}
//
//static TK_API_RESULT tk_core_executeProgramSwitchCommand(
...
static TK_API_RESULT tk_core_getArguments(
...
static TK_API_RESULT tk_core_executeProgramCommand(
    tk_core_Topbar *Topbar_p, tk_core_MicroWindow *Window_p)
{
    ...
    if (Program_p->Prototype_p->Callbacks.handleCommand_f != NULL) 
    {
        ...
        TK_API_RESULT result = Program_p->Prototype_p->Callbacks.handleCommand_f(Program_p);

        switch (result)
        {
            case TK_API_ERROR_UNKNOWN_COMMAND :
                TK_CHECK(tk_core_setDefaultMessage(Topbar_p, TK_CORE_MESSAGE_MISC_UNKNOWN_COMMAND))
                break;
             case TK_API_ERROR_INVALID_ARGUMENT :
...
TK_API_RESULT tk_core_handleTopbarMouseInput(
...
TK_API_RESULT tk_core_handleTopbarKeyboardInput(
...
//}
//
//static TK_API_RESULT tk_core_drawTopbarCommandSuggestion(
...
//}
//
//static TK_API_RESULT tk_core_drawTopbarProgramSuggestion(
//    tk_core_Topbar *Topbar_p, tk_core_MicroWindow *Window_p, tk_core_Glyph *Glyphs_p, int cols)
//{
//    Glyphs_p[2] = tk_core_getGlyphHelper(':');
//    Glyphs_p[3] = tk_core_getGlyphHelper(' ');
...
//}
//
//static TK_API_RESULT tk_core_setSuggestionIndex(
...
//}
//
//static TK_API_RESULT tk_core_drawTopbarSuggestion(
//    tk_core_Topbar *Topbar_p, tk_core_MicroWindow *Window_p, tk_core_Glyph *Glyphs_p, int cols)
//{
...
static TK_API_RESULT tk_core_drawTopbarMessage(
    tk_core_Config *Config_p, tk_core_Topbar *Topbar_p, tk_core_MicroWindow *Window_p, tk_core_Glyph *Glyphs_p, int cols)
{
    if (Config_p->Topbar.on && Topbar_p->Message.Text.length == 0) {
        tk_core_Program *Program_p = tk_core_getCurrentProgram(Window_p);
        if (!Program_p || !Program_p->Prototype_p) {return TK_API_SUCCESS;}
...
static TK_API_RESULT tk_core_drawTopbarText(
    tk_core_Config *Config_p, tk_core_Tile *Tile_p, tk_core_Glyph *Glyphs_p, int cols)
{
    ...
    if (!Topbar_p->hasFocus) 
    {
        ...
        tk_core_MacroWindow *MacroWindow_p = Session_p->Window_p;
    ...
    tk_core_Tile *Tile_p, tk_core_Glyph *Glyphs_p, int cols, int row, bool standardIO)
{
    ...
    if (Tile_p) {
        TK_CHECK(tk_core_drawTopbarText(&Session_p->Config, Tile_p, Glyphs_p, cols))
    }
...
TK_API_RESULT tk_core_updateTopbar(
    tk_core_Topbar *Topbar_p)
{
