...
static bool tk_core_pathMatchesContextMenu(
    tk_core_ContextMenu *Menu_p, const NH_API_UTF32 **pp, int count)
{
...
static int tk_core_isContextMenuTiling(
    tk_core_ContextMenu *Menu_p)
{
    ...
    for (int i = 0; i < 4; ++i) {
        if (tk_core_pathMatchesContextMenu(Menu_p, paths_ppp[i], 2)) {
            return i;
        }
    }
...
static int tk_core_isContextMenuWindowOrTabSelect(
    tk_core_ContextMenu *Menu_p)
{
    static const NH_API_UTF32 window_p[] = {'W', 'i', 'n', 'd', 'o', 'w', 0};
    static const NH_API_UTF32 tab_p[] = {'T', 'a', 'b', 0};
...
static int tk_core_handleContextMenuTiling(
    int action, tk_core_Tile *Tile_p, int cCol, int cRow)
{
    nh_api_KeyboardEvent Event;
    tk_core_MacroWindow *Window_p = ((tk_core_Session*)nh_core_getWorkloadArg())->Window_p;
...
static tk_core_ContextMenu *tk_core_parseContextMenu(
    ...
    tk_core_ContextMenu *Menu_p)
{
    for (int i = 0; i < Menu_p->Items.size; ++i) {
        tk_core_freeContextMenu(Menu_p->Items.pp[i]);
    }
...
static void tk_core_computeContextMenuPosition(
    tk_core_ContextMenu *Menu_p, int x, int y, int maxX, int maxY)
{
    // Get menu width.
    int width = 0;
    for (int i = 0; i < Menu_p->Items.size; ++i) {
        nh_encoding_UTF32String *Name_p = &((tk_core_ContextMenu*)Menu_p->Items.pp[i])->Name;
    ...
    for (int i = 0; i < Menu_p->Items.size; ++i) {
        tk_core_computeContextMenuPosition(Menu_p->Items.pp[i], x + width + 2, Menu_p->Position.y + i, maxX, maxY);
    }

    return;
}

static bool tk_core_compareContextMenuName(
    tk_core_ContextMenu *Menu_p, char *name_p)
{
    ...
    tk_core_ContextMenu *Menu_p, tk_core_ContextMenu *Parent_p, bool recursive, int x, int y)
{
    ...
    for (int i = 0; Parent_p && i < Parent_p->Items.size; ++i) {
        nh_encoding_UTF32String *Name_p = &((tk_core_ContextMenu*)Parent_p->Items.pp[i])->Name;
        if (Name_p->length > width) {width = Name_p->length;}
    ...
    if (y2 == y && Parent_p && Parent_p->Position.x <= x2 && (Parent_p->Position.x + width) >= x2) {
        ...
        int maxCols = ((tk_core_View*)Session_p->Views.pp[0])->cols;
        int width2 = 0;
        for (int i = 0; i < Menu_p->Items.size; ++i) {
            nh_encoding_UTF32String *Name_p = &((tk_core_ContextMenu*)Menu_p->Items.pp[i])->Name;
    ...
    for (int i = 0; (Menu_p->active || Menu_p->hit) && i < Menu_p->Items.size && recursive; ++i) {
        tk_core_ContextMenu *Result_p = tk_core_isContextMenuHit(Menu_p->Items.pp[i], Menu_p, recursive, x, y);
    ...
    tk_core_ContextMenu *Menu_p, tk_core_ContextMenu *Parent_p, int x, int y, bool activate)
{
    ...
    Menu_p->hit = tk_core_isContextMenuHit(Menu_p, Parent_p, false, x, y) != NULL && Menu_p->Name.length > 0;
    ...
    if (Menu_p->active && Parent_p) {
        for (int i = 0; i < Parent_p->Items.size; ++i) {
            ((tk_core_ContextMenu*)Parent_p->Items.pp[i])->active = Parent_p->Items.pp[i] == Menu_p;
            ((tk_core_ContextMenu*)Parent_p->Items.pp[i])->hit = Parent_p->Items.pp[i] == Menu_p;
        }
    }
    ...
    for (int i = 0; (Menu_p->active || Menu_p->hit) && i < Menu_p->Items.size; ++i) {
        tk_core_updateContextMenuHit(Menu_p->Items.pp[i], Menu_p, x, y, activate);
        if (((tk_core_ContextMenu*)Menu_p->Items.pp[i])->hit) {Menu_p->hit = true;}
    }
...
{
    tk_core_Session *Session_p = nh_core_getWorkloadArg();
    tk_core_Program *Program_p = tk_core_getCurrentProgram(&TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p)->MicroWindow);
    ...
    if (Session_p->Config.Menu.program && Session_p->Prototypes.size > 1) {
        tk_core_Program *Program_p = tk_core_getCurrentProgram(&TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p)->MicroWindow);
        ...
        for (int i = 0; i < Session_p->Prototypes.size; ++i) {
            ...
            if (i < Session_p->Prototypes.size-1) {nh_encoding_appendUTF32Codepoint(&Menu, ',');}
        }
        nh_encoding_appendUTF32Codepoint(&Menu, '}');
        nh_encoding_appendUTF32Codepoint(&Menu, ',');
        border = true;
    ...
    if (Session_p->Config.Menu.split) {
        if (border) {
            nh_encoding_appendUTF32Codepoint(&Menu, ',');
        } 
    ...
    if (Session_p->Config.Menu.append) {
        if (border && !Session_p->Config.Menu.split) {
            nh_encoding_appendUTF32Codepoint(&Menu, ',');
        } 
        NH_API_UTF32 tiling_p[] = {
            'A', 'p', 'p', 'e', 'n', 'd', '{',
...
//            if (nh_core_getListIndex(&Session_p->Windows, Session_p->Window_p) == i) {
//                tmp2_p[2] = 0x2022;
//            }
...
//            NH_API_UTF32 tmp2_p[] = {i + '1', 1, ' '};
//            if (((tk_core_MacroTile*)Session_p->Window_p->Tile_p->p)->current == i) {
//                tmp2_p[2] = 0x2022;
//            }
//            nh_encoding_appendUTF32(&Menu, tmp2_p, 3);
//            if (i < (Config.tabs-1)) {
...
//    }
    ...
    if (1) {
        ...
        tk_core_Glyph Glyph = ((tk_core_View*)Session_p->Views.pp[0])->Grid1_p[y].Glyphs_p[x];
    ...
    if (Session_p->Config.Menu.close) {
        if (border) {
            nh_encoding_appendUTF32Codepoint(&Menu, ',');
        } 
    ...
    tk_core_computeContextMenuPosition(Menu_p, x, y, ((tk_core_View*)Session_p->Views.pp[0])->cols, ((tk_core_View*)Session_p->Views.pp[0])->rows);
...
TK_API_RESULT tk_core_handleMouseMenuPress(
    tk_core_ContextMenu *Root_p, tk_core_ContextMenu *Menu_p)
{
    if (!Menu_p->Parent_p || Menu_p->Name.length == 0) {return TK_API_SUCCESS;}

    tk_core_Session *Session_p = nh_core_getWorkloadArg();

    // Handle tiling or tabing if necessary.
    int tiling = tk_core_isContextMenuTiling(Menu_p);
    if (tiling >= 0) {
        if (tiling == 1 || tiling == 3) { 
            tk_core_handleContextMenuTiling(tiling, Session_p->Window_p->Tile_p, Root_p->cCol, Root_p->cRow);
        } else {
            tk_core_handleContextMenuTiling(tiling, TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p))->Tile_p, Root_p->cCol, Root_p->cRow);
        }
        nh_api_KeyboardEvent Event;
        Event.trigger = NH_API_TRIGGER_PRESS;
        Event.codepoint = 13;
        return tk_core_handleTilingInput(Session_p->Window_p, Event);
    }

    int select = tk_core_isContextMenuWindowOrTabSelect(Menu_p);
    if (select >= 0) {
        if (select < 9) {
            TK_CHECK_NULL(tk_core_insertAndFocusWindow(Session_p, select))
        } else {
            ((tk_core_MacroTile*)Session_p->Window_p->Tile_p->p)->current = select - 9; 
        }
    ...
    if (nh_encoding_compareUTF32(Menu_p->Parent_p->Name.p, program_p)) {
        for (int i = 0; i < Menu_p->Parent_p->Items.size; ++i) {
            ...
            if (Prog_p->active) {
                TK_CORE_MACRO_TAB(((tk_core_Session*)nh_core_getWorkloadArg())->Window_p->Tile_p)->MicroWindow.current = i;
            } 
        }
    } else if (nh_encoding_compareUTF32(Menu_p->Name.p, close_p)) {
        TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p))->Tile_p->close = true;
    } else {
...
TK_API_RESULT tk_core_drawContextMenuRecursively(
