...
static TK_API_RESULT tk_core_getMaxCursorPosition(
...
static TK_API_RESULT tk_core_readLinuxStandardInput(
    NH_API_UTF32 codepoints_p[4], int *count_p)
{
    ...
    if (rv == -1 || rv == 0) {return TK_API_SUCCESS;}
    ...
    tk_core_Session *Session_p)
{
    ...
    for (int i = 0; i < count; ++i) {
        nh_api_WSIEvent *Event_p = (nh_api_WSIEvent*)nh_core_advanceRingBuffer(&Session_p->Events);
...
static TK_API_RESULT tk_core_enterRawMode(
    tk_core_View *View_p) 
{
#ifdef __unix__
...
static TK_API_RESULT tk_core_exitRawMode(
    tk_core_View *View_p) 
{
#ifdef __unix__
...
TK_API_RESULT tk_core_claimStandardIO(
    tk_core_Session *Session_p)
{
    if (claimed) {return TK_API_ERROR_BAD_STATE;}

    tk_core_View *View_p = tk_core_createView(Session_p, NULL, true);
    TK_CHECK_NULL(View_p)

    TK_API_RESULT error = tk_core_enterRawMode(View_p);
    if (error) {
        tk_core_destroyView(Session_p, View_p);
        return error;
    }
...
{
    for (int i = 0; i < Session_p->Views.size; ++i) {
        if (((tk_core_View*)Session_p->Views.pp[i])->standardIO) {return true;}
    }
    return false;
}
