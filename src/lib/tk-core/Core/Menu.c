...
TK_API_RESULT tk_core_drawMicroWindowMenu(
    tk_core_Row *Grid_p)
{
    ...
    // Get menu height.
    int height = TK_CORE_MACRO_TILE(Session_p->Window_p->Tile_p)->MacroTabs.size;
    if (height <= 0) {return TK_API_SUCCESS;}

    nh_api_PixelPosition Position;
    Position.x = Session_p->Window_p->Tile_p->colPosition+Session_p->Window_p->Tile_p->colSize/2;
    Position.y = Session_p->Window_p->Tile_p->rowPosition+Session_p->Window_p->Tile_p->rowSize/2-height/2;

    if (Session_p->Config.Titlebar.on) {
        Position.x+=2;
    }

    // Get menu width.
    int width = 0;
    for (int i = 0; i <  TK_CORE_MACRO_TILE(Session_p->Window_p->Tile_p)->MacroTabs.size; ++i) {
        tk_core_Program *Program_p = tk_core_getCurrentProgram(&((tk_core_MacroTab*)((tk_core_MacroTile*)Session_p->Window_p->Tile_p->p)->MacroTabs.pp[i])->MicroWindow);
    ...
    if (width <= 0) {return TK_API_SUCCESS;}

    width += 4;

    Position.x -= width/2;
    Position.y -= Session_p->Windows.size/2;

    Session_p->TabSwitchMenu.Position = Position;
    Session_p->TabSwitchMenu.width = width;
    Session_p->TabSwitchMenu.height = height;
...
TK_API_RESULT tk_core_drawPrompt(
    tk_core_Row *Grid_p)
{
    tk_core_Session *Session_p = nh_core_getWorkloadArg();
    tk_core_View *View_p = Session_p->Views.pp[0];
...
//        tk_core_Program *Program_p = tk_core_getCurrentProgram(&TK_CORE_MACRO_TAB(((tk_core_MacroWindow*)Session_p->Windows.pp[i])->Tile_p)->MicroWindow);
//        if (!Program_p || !Program_p->Prototype_p) {continue;}
//        NH_API_UTF32 title_p[255] = {0};
//        Program_p->Prototype_p->Callbacks.getTitle_f(Program_p, title_p, 255);
//        int length = 0;
...
//    width += 4;
    ...
    Session_p->WindowSwitchMenu.Position = Position;
    Session_p->WindowSwitchMenu.width = width;
    Session_p->WindowSwitchMenu.height = height;
...
//            Grid_p[row].Glyphs_p[Position.x+k+3] = tk_core_getGlyphHelper(title_p[j]);
//        }
//        if (Session_p->Window_p == Session_p->Windows.pp[i]) {
//            Grid_p[row].Glyphs_p[Position.x+1] = tk_core_getGlyphHelper(0x25cf);
//        }
//    }
