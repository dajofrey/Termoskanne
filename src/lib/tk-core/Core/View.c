...
#include "../../tk-gfx/Renderer/Grid.h"
...
TK_API_RESULT tk_core_getViewSize(
    tk_core_View *View_p) 
{
...
TK_API_RESULT tk_core_translateMousePosition(
    tk_core_View *View_p, nh_api_MouseEvent Mouse, int *col_p, int *row_p)
{
    if (View_p->standardIO) {
        TK_CHECK(TK_API_ERROR_BAD_STATE)
    }
...
TK_API_RESULT tk_core_updateView(
    ...
    nh_core_RingBuffer *Buffer_p, int itemCount)
{
    for (int i = 0; i < itemCount; ++i) {
        ...
        *Array_p = nh_core_initArray(sizeof(tk_gfx_TileUpdate), 255);
    }
}
    ...
    nh_core_RingBuffer *Buffer_p, int itemCount)
{
    for (int i = 0; i < itemCount; ++i) {
        nh_core_Array *Array_p = (nh_core_Array*)nh_core_advanceRingBuffer(Buffer_p);
        *Array_p = nh_core_initArray(sizeof(tk_gfx_Box), 16);
    }
}

tk_core_View *tk_core_createView(
    tk_api_Session *Session_p, void *p, bool standardIO)
{
    tk_core_View View;
    ...
    *View_p = View;
    ...
    if (Session_p) {
        nh_core_appendToList(&((tk_core_Session*)Session_p)->Views, View_p);
    }

    return View_p;
}

TK_API_RESULT tk_core_destroyView(
    tk_api_Session *Session_p, tk_core_View *View_p)
{
    for (int i = 0; i < 64; ++i) {
    ...
    if (Session_p) {
        nh_core_removeFromList2(&((tk_core_Session*)Session_p)->Views, true, View_p);
    } else {
        nh_core_free(View_p); 
    }
...
TK_API_RESULT tk_core_forwardCursor(
...
static TK_API_RESULT tk_core_setContextMenuBoxes(
    tk_core_ContextMenu *Menu_p, nh_core_Array *Boxes_p)
{
    if (!Menu_p->active || Menu_p->Items.size == 0) {return TK_API_SUCCESS;}

    tk_gfx_Box *Box_p = (tk_gfx_Box*)nh_core_incrementArray(Boxes_p);
    ...
    memset(Box_p, 0, sizeof(tk_gfx_Box));
...
static TK_API_RESULT tk_core_setBox(
...
TK_API_RESULT tk_core_forwardGrid1(
...
TK_API_RESULT tk_core_forwardGrid2(
    tk_core_View *View_p)
{
    nh_core_Array *Array_p = (nh_core_Array*)nh_core_advanceRingBuffer(&View_p->Forward.Tiles);
    nh_core_freeArray(Array_p);
...
TK_API_RESULT tk_core_forwardBoxes(
    tk_core_Config *Config_p, tk_core_View *View_p)
{
    ...
    if (Session_p->alt && Session_p->ctrl) {
        tk_core_setBox(Session_p->TabSwitchMenu, Boxes_p);
    }

    // Forward boxes: Context Menu.
    if (Session_p->Window_p->MouseMenu_p) {
        tk_core_setContextMenuBoxes(Session_p->Window_p->MouseMenu_p, Boxes_p);
    }

    // Forward boxes: Inactive cursor outlines.
    nh_core_List MacroTiles = tk_core_getTiles(Session_p->Window_p->RootTile_p);
    for (int i = 0; i < MacroTiles.size; ++i) {
        ...
        for (int j = 0; j < MicroTiles.size; ++j) {
            ...
            tk_core_Program *CurrentProgram_p = tk_core_getCurrentProgram(&TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p)->MicroWindow);
            if (Program_p == NULL || CurrentProgram_p == Program_p && Session_p->hasFocus) {continue;}
...
TK_API_RESULT tk_core_forwardEvent(
