...
TK_API_RESULT tk_core_closeTile(
...
static TK_API_RESULT tk_core_leaveTilingAndFocusTile(
...
static TK_API_RESULT tk_core_computeTileSizeRecursively(
...
TK_API_RESULT tk_core_updateTiling(
...
static TK_API_RESULT tk_core_insertEmptyTile(
        ...
        case TK_CORE_INSERT_TILE_LEFT_KEY :
            if (Parent_p->type == TK_CORE_TILE_TYPE_MACRO) {
                if (Parent_p->Children.count == 0) {
                    ...
                    Session_p->InsertTile_p     = tk_core_createMacroTile(Parent_p, NULL, 0);
                    Session_p->Window_p->Tile_p = tk_core_createMacroTile(Parent_p, List_pp, 1);
                ...
                if (Parent_p->Children.count == 0) {
                    Session_p->InsertTile_p = tk_core_createMicroTile(Parent_p, NULL, 0);
        ...
        case TK_CORE_INSERT_TILE_RIGHT_KEY  :
            if (Parent_p->type == TK_CORE_TILE_TYPE_MACRO) {
                if (Parent_p->Children.count == 0) {
                    ...
                    Session_p->Window_p->Tile_p = tk_core_createMacroTile(Parent_p, List_pp, 0);
                    Session_p->InsertTile_p     = tk_core_createMacroTile(Parent_p, NULL, 1);
                    // Remove data references from parent tile.
                    nh_core_freeList(&TK_CORE_MACRO_TILE(Parent_p)->MacroTabs, true);
                    TK_CORE_MACRO_TILE(Parent_p)->current = -1;
                } else {
                    Session_p->InsertTile_p = tk_core_createMacroTile(Parent_p, NULL, Parent_p->Children.count);
                }
            } else {
                if (Parent_p->Children.count == 0) {
                    tk_core_createMicroTile(Parent_p, TK_CORE_MICRO_TILE(Parent_p)->Program_p, 0);
                    Session_p->InsertTile_p = tk_core_createMicroTile(Parent_p, NULL, 1);
                    // Remove data references from parent tile.
                    TK_CORE_MICRO_TILE(Parent_p)->Program_p = NULL;
                } else {
                    Session_p->InsertTile_p = tk_core_createMicroTile(Parent_p, NULL, Parent_p->Children.count);
                }
            }
            break;
    }
...
static TK_API_RESULT tk_core_addTile(
    tk_core_Tile *Tile_p, NH_API_UTF32 c)
{
    if (Tile_p->Parent_p == NULL || Tile_p->Parent_p->Children.count == 0) {return TK_API_ERROR_BAD_STATE;}
    return tk_core_insertEmptyTile(Tile_p->Parent_p, c);
}

static TK_API_RESULT tk_core_splitTile(
    tk_core_Tile *Tile_p, NH_API_UTF32 c)
{
    if (Tile_p->Children.count != 0) {return TK_API_ERROR_BAD_STATE;}
    return tk_core_insertEmptyTile(Tile_p, c);
}

// RESET ===========================================================================================

TK_API_RESULT tk_core_resetTiling(
    ...
    if (Window_p->Tiling.mode == TK_CORE_TILING_MODE_MICRO) {
        switch (Window_p->Tiling.stage)
        {
            ...
            case TK_CORE_TILING_STAGE_INSERT :
                tk_core_closeTile(Session_p->InsertTile_p, TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Window_p->Tile_p)));
                TK_CHECK(tk_core_leaveTilingAndFocusTile(Window_p, TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Window_p->Tile_p))->LastFocus_p))
                break;
        }
    } else {
        switch (Window_p->Tiling.stage)
        {
            ...
            case TK_CORE_TILING_STAGE_INSERT :
                tk_core_closeTile(Session_p->InsertTile_p, Window_p);
                TK_CHECK(tk_core_leaveTilingAndFocusTile(Window_p, Window_p->LastFocus_p))
                break;
        }
    }
...
static TK_API_RESULT tk_core_handlePotentialMacroTileInsertion(
    tk_core_MacroWindow *Window_p, NH_API_UTF32 c)
{
    ...
    if (c == 13) 
    {
        ...
        for (int i = 0; i < 9; ++i) {
            TK_CORE_MACRO_TAB_2(Session_p->InsertTile_p, i)->MicroWindow.Tabs_p = tk_core_createMicroTabs(Session_p);
            TK_CORE_MACRO_TAB_2(Session_p->InsertTile_p, i)->MicroWindow.current = 0; 
        }

        return tk_core_leaveTilingAndFocusTile(Window_p, Session_p->InsertTile_p);
    }
    ...
    switch (c) 
    {
        ...
        case TK_CORE_INSERT_TILE_RIGHT_KEY  :
            ...
            if (Session_p->InsertTile_p) { 
                tk_core_moveInsertTile(Session_p->InsertTile_p, c);
                break;
            }
            // Fall through.
...
static TK_API_RESULT tk_core_handlePotentialMicroTileInsertion(
    tk_core_MacroWindow *Window_p, NH_API_UTF32 c)
{
    tk_core_Session *Session_p = nh_core_getWorkloadArg();
    ...
    if (c == 13)
    {
        ...
        TK_CORE_MICRO_TILE(Session_p->InsertTile_p)->Program_p = tk_core_createProgramInstance(
...
static TK_API_RESULT tk_core_handleMacroTilingInput(
...
static TK_API_RESULT tk_core_handleMicroTilingInput(
    tk_core_MacroWindow *Window_p, nh_api_KeyboardEvent Event)
{
    NH_API_UTF32 c = Event.codepoint;
...
TK_API_RESULT tk_core_handleTilingInput(
    tk_core_MacroWindow *Window_p, nh_api_KeyboardEvent Event)
{
