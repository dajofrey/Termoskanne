...
TK_API_RESULT tk_core_getCursorPosition(
...
TK_API_RESULT tk_core_refreshCursor(
    tk_core_Session *Session_p) 
{
    tk_core_View *View_p = Session_p->Views.pp[0];
    int x = -1, y = -1;
 
    if (Session_p->hasFocus) {
        tk_core_Tile *MicroTile_p = TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p))->Tile_p;
        TK_CHECK(tk_core_getCursorPosition(&Session_p->Config, Session_p->Window_p->Tile_p, MicroTile_p, View_p->standardIO, &x, &y))
    }

    TK_CHECK(tk_core_forwardCursor(&Session_p->Config, View_p, x, y))
...
static TK_API_RESULT tk_core_drawMicroTile(
    tk_core_Config *Config_p, tk_core_Tile *Tile_p, tk_core_View *View_p, int row)
{
    ...
    if (Tile_p != TK_CORE_MICRO_TAB(TK_CORE_MACRO_TAB(Session_p->Window_p->Tile_p))->Tile_p) {
...
static TK_API_RESULT tk_core_drawMacroTile(
    tk_core_Config *Config_p, tk_core_Tile *Tile_p, tk_core_View *View_p, int row)
{
    // Get relative row with 0 being the first row of the tile.
    row = row - Tile_p->rowPosition;
...
static TK_API_RESULT tk_core_draw(
    tk_core_Config *Config_p, tk_core_Tile *Tile_p, tk_core_View *View_p, int row)
{
    // Normalize glyphs.
        ...
        if (Glyph_p->mark & TK_CORE_MARK_LINE_VERTICAL) {
            ...
            if ((Glyph_p+1)->mark & TK_CORE_MARK_LINE_HORIZONTAL) {
                if ((Glyph_p-1)->mark & TK_CORE_MARK_LINE_HORIZONTAL) {
                    ...
                    if (Glyph_p->overlay && i > 0 && (Glyph_p-1)->mark & TK_CORE_MARK_ACCENT_BACKGROUND_2) {
...
TK_API_RESULT tk_core_refreshGrid1Row(
...
TK_API_RESULT tk_core_refreshGrid1(
    tk_core_Session *Session_p) 
{
    tk_core_View *View_p = Session_p->Views.pp[0];

    if (Session_p->Config.Sidebar.on) {
        View_p->cols -= 2;
    }

    tk_core_updateTiling(Session_p->Window_p->RootTile_p, View_p->rows, View_p->cols);
    ...
    for (int row = 0; row < View_p->rows; ++row) {
        ...
        TK_CHECK(tk_core_refreshGrid1Row(&Session_p->Config, &Tiles, View_p, row))
    ...
    if (Session_p->Config.Sidebar.on) {
        View_p->cols += 2;
        tk_core_drawSidebar(Session_p);
    }

    nh_core_freeList(&Tiles, false);
    TK_CHECK(tk_core_forwardGrid1(&Session_p->Config, View_p))

    return TK_API_SUCCESS;
}

TK_API_RESULT tk_core_refreshGrid2(
    tk_core_Session *Session_p)
{
    tk_core_View *View_p = Session_p->Views.pp[0];
...
//    }
    ...
    TK_CHECK(tk_core_drawContextMenuRecursively(Session_p->Window_p->MouseMenu_p, View_p->Grid2_p))
    TK_CHECK(tk_core_forwardGrid2(View_p))
    TK_CHECK(tk_core_forwardBoxes(&Session_p->Config, View_p))
