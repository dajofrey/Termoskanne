    ...
    void *Session_p)
{
    ...
    for (int i = 0; i < 9; ++i) {
        MicroTabs_pp[i] = tk_core_createMicroTabs(Session_p);
...
static TK_API_RESULT tk_core_destroyMacroWindow(
...
tk_core_MacroWindow *tk_core_insertAndFocusWindow(
    void *Session_p, int index)
{
    // Close mouse menu.
    if (((tk_core_Session*)Session_p)->Window_p && ((tk_core_Session*)Session_p)->Window_p->MouseMenu_p) {
        tk_core_freeContextMenu(((tk_core_Session*)Session_p)->Window_p->MouseMenu_p);
        ((tk_core_Session*)Session_p)->Window_p->MouseMenu_p = NULL;
    }

    tk_core_MacroWindow *Window_p = nh_core_getFromList(&((tk_core_Session*)Session_p)->Windows, index);
    if (Window_p) {
        // Only focus window.
        ((tk_core_Session*)Session_p)->Window_p = Window_p;
        return Window_p;
    } 

    for (int i = ((tk_core_Session*)Session_p)->Windows.size; i <= index; ++i) {
        Window_p = tk_core_createMacroWindow(Session_p);
        TK_CHECK_NULL_2(NULL, Window_p)
        nh_core_appendToList(&((tk_core_Session*)Session_p)->Windows, Window_p);
...
TK_API_RESULT tk_core_destroyWindows(
    void *Session_p)
{
    for (int i = 0; i < ((tk_core_Session*)Session_p)->Windows.size; ++i) {
        tk_core_destroyMacroWindow(((tk_core_Session*)Session_p)->Windows.pp[i]);
    }

    ((tk_core_Session*)Session_p)->Window_p = NULL;

    nh_core_freeList(&((tk_core_Session*)Session_p)->Windows, false);
...
static TK_API_RESULT tk_core_updateMacroTile(
...
TK_API_RESULT tk_core_updateMacroWindow(
    tk_core_MacroWindow *Window_p)
{
    // Get macro tiles.
...
static bool tk_core_isTilingInput(
    tk_core_Session *Session_p, nh_api_KeyboardEvent Event)
{
    bool tiling = false;

    if (Session_p->Window_p->Tiling.stage > 0) {
        tiling = true;
    ...
    if (Session_p->Prototypes.size == 0) {
        tiling = false;
    }
...
{
    int i = 0;
    for (; i < ((tk_core_Session*)Session_p)->Windows.size && ((tk_core_Session*)Session_p)->Windows.pp[i] != ((tk_core_Session*)Session_p)->Window_p; ++i);
    return i;
}

static TK_API_RESULT tk_core_handleKeyboardInput(
...
//    if (nh_encoding_isASCIIDigit(Event.Keyboard.codepoint) && Event.Keyboard.trigger == NH_API_TRIGGER_PRESS && Session_p->alt && Session_p->ctrl) {
//        if (Config.windows > Event.Keyboard.codepoint - '1') {
//            Window_p = tk_core_insertAndFocusWindow(Session_p, Event.Keyboard.codepoint - '1');
...
//    }
    if (nh_encoding_isASCIIDigit(Event.Keyboard.codepoint) && Event.Keyboard.trigger == NH_API_TRIGGER_PRESS && (Session_p->alt || Session_p->ctrl)) {
        if (9 > Event.Keyboard.codepoint - '1') {
            ...
            if (!Tab_p) {return TK_API_ERROR_BAD_STATE;}
            Tab_p->active = true;
        }
        return TK_API_SUCCESS;
    }
    ...
    if (Event.Keyboard.special == NH_API_KEY_CONTROL_L && Event.Keyboard.trigger == NH_API_TRIGGER_RELEASE) {
        Session_p->ctrl = false;
        Window_p->refreshGrid1 = true;
        Window_p->refreshGrid2 = true;
    }

    if (tk_core_isTilingInput(Session_p, Event.Keyboard)) {
        TK_CHECK(tk_core_handleTilingInput(Window_p, Event.Keyboard))
        return TK_API_SUCCESS;
    }

    // Switch tile.
    if (Event.Keyboard.trigger == NH_API_TRIGGER_PRESS && (Session_p->alt || (Session_p->alt && Session_p->ctrl))) {
        ...
        if (New_p) {
            ...
            if (Event.Keyboard.codepoint == 'd' || (Event.Keyboard.codepoint == CTRL_KEY('d') && Session_p->ctrl && Session_p->alt)) {
                Window_p = tk_core_insertAndFocusWindow(Session_p, tk_core_getCurrentWindowIndex(Session_p)+1);
            } else if (Event.Keyboard.codepoint == 'a' || (Event.Keyboard.codepoint == CTRL_KEY('a') && Session_p->ctrl && Session_p->alt)) {
                int window = tk_core_getCurrentWindowIndex(Session_p);
                if (window > 0) {
                    Window_p = tk_core_insertAndFocusWindow(Session_p, window-1);
                }
            }
        }
    }
...
static TK_API_RESULT tk_core_handleMouseInput(
    tk_core_Config *Config_p, tk_core_MacroWindow *Window_p, nh_api_WSIEvent Event, int col, int row)
{
    ...
    if (row == -1 && Config_p->Titlebar.on == true) {
        ...
        if (Event.Mouse.trigger == NH_API_TRIGGER_PRESS) {
            tk_core_handleTitlebarHit(Session_p, Event.Mouse, cCol);
            Session_p->Window_p->refreshGrid2 = true;
        }
    }
 
    if (Event.Mouse.type == NH_API_TRIGGER_MOVE) {
        bool refresh = tk_core_handleSidebarMove(Session_p, cCol, cRow);
        if (refresh) {Window_p->refreshGrid1 = true;}
        refresh = tk_core_handleTitlebarMove(Session_p, cCol, cRow);
        if (refresh) {Window_p->refreshGrid1 = true;}
    }

    if (Event.Mouse.type == NH_API_MOUSE_LEFT && Event.Mouse.type == NH_API_TRIGGER_PRESS) {
        if (Session_p->Config.Sidebar.on && (cCol == -2 || cCol == -1)) {
	    tk_core_handleSidebarHit(Session_p, Event.Mouse, cRow);
	}
    }
    ...
    if (MacroTile_p == NULL || MicroTile_p == NULL) {return TK_API_SUCCESS;}
    ...
    if (Window_p->MouseMenu_p) {
        ...
        if (Hit_p) {
            ...
            Window_p->refreshGrid2 = true;
            ((tk_core_Session*)nh_core_getWorkloadArg())->Window_p->refreshGrid2 = true; // Focused Window might have changed.
            goto SKIP;
        }
    }
...
TK_API_RESULT tk_core_handleMacroWindowInput(
    tk_core_Config *Config_p, tk_core_MacroWindow *Window_p, nh_api_WSIEvent Event)
{
    switch (Event.type)
    {
        ...
        case NH_API_WSI_EVENT_MOUSE :
        {
            int col = -1;
            int row = -1;
            tk_core_translateMousePosition(((tk_core_Session*)nh_core_getWorkloadArg())->Views.pp[0], Event.Mouse, &col, &row);
