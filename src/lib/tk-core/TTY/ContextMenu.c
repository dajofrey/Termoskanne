    ...
    int action, tk_core_Tile *Tile_p, int cCol, int cRow)
{
    ...
    tk_core_MacroWindow *Window_p = ((tk_core_TTY*)nh_core_getWorkloadArg())->Window_p;
    ...
    tk_core_ContextMenu *Menu_p, tk_core_ContextMenu *Parent_p, bool recursive, int x, int y)
{
    ...
    if (y2 == y && Parent_p && Parent_p->Position.x <= x2 && (Parent_p->Position.x + width) >= x2) {
        ...
        int maxCols = ((tk_core_View*)TTY_p->Views.pp[0])->cols;
        int width2 = 0;
        for (int i = 0; i < Menu_p->Items.size; ++i) {
            nh_encoding_UTF32String *Name_p = &((tk_core_ContextMenu*)Menu_p->Items.pp[i])->Name;
    ...
    int x, int y)
{
    ...
    if (TTY_p->Config.Menu.program && TTY_p->Prototypes.size > 1) {
        ...
        for (int i = 0; i < TTY_p->Prototypes.size; ++i) {
            if (nh_encoding_getUTF32Length(((tk_core_Interface*)TTY_p->Prototypes.pp[i])->name_p) > width) {
                width = nh_encoding_getUTF32Length(((tk_core_Interface*)TTY_p->Prototypes.pp[i])->name_p);
            }
        }
        for (int i = 0; i < TTY_p->Prototypes.size; ++i) {
    ...
    if (TTY_p->Config.Menu.append) {
        if (border && !TTY_p->Config.Menu.split) {
            nh_encoding_appendUTF32Codepoint(&Menu, ',');
        } 
        NH_API_UTF32 tiling_p[] = {
            'A', 'p', 'p', 'e', 'n', 'd', '{',
    ...
    tk_core_ContextMenu *Root_p, tk_core_ContextMenu *Menu_p)
{
    ...
    if (tiling >= 0) {
        ...
        Event.trigger = NH_API_TRIGGER_PRESS;
        Event.codepoint = 13;
        return tk_core_handleTilingInput(TTY_p->Window_p, Event);
    }
