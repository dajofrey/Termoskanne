...
static TK_API_RESULT tk_core_getSetting(
...
static tk_core_Config tk_core_getStaticConfig()
{
    ...
    for (int i = 0; i < options; ++i) {
        TK_CHECK_2(Config, tk_core_getSetting(&Config, Session_p->namespace_p, i, options_pp[i]))
    }
...
tk_core_Config tk_core_updateConfig( 
    void *Session_p) 
{ 
    ((tk_core_Session*)Session_p)->Config = tk_core_getStaticConfig(); 
    if (((tk_core_Session*)Session_p)->Views.size > 0 && ((tk_core_Session*)Session_p)->Views.pp != NULL && ((tk_core_View*)((tk_core_Session*)Session_p)->Views.pp[0])->standardIO) {
        ((tk_core_Session*)Session_p)->Config.Titlebar.on = false;
        ((tk_core_Session*)Session_p)->Config.Topbar.on = false;
        ((tk_core_Session*)Session_p)->Config.Sidebar.on = false;
    }
