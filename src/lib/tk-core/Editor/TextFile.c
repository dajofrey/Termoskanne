...
static TK_API_RESULT tk_core_appendToTextFileLine(
    tk_core_TextFileLine *Line_p, NH_API_UTF32 *codepoints_p, int length)
{
    NH_CORE_CHECK_2(TK_API_ERROR_BAD_STATE, nh_encoding_appendUTF32(&Line_p->Codepoints, codepoints_p, length))
    bool b = false;
...
{
    NH_CORE_CHECK_2(TK_API_ERROR_BAD_STATE, nh_encoding_insertUTF32(&Line_p->Codepoints, index, &c, 1))
    bool b = false;
...
{
    NH_CORE_CHECK_2(TK_API_ERROR_BAD_STATE, nh_encoding_removeUTF32(&Line_p->Codepoints, index, length))
    ...
    nh_encoding_UTF32String *Path_p)
{
    ...
    if (p) 
    {
        TextFile_p->textType = tk_core_getTextType(Path_p);
    ...
    tk_core_TextFile *File_p, nh_encoding_UTF32String *Path_p)
{
    ...
    {
        tk_core_TextFileLine *Line_p = File_p->Lines.pp[i];
        bool render = false;
...
static tk_core_Glyph *tk_core_setNextGlyph(
    tk_core_Glyph **Glyphs_pp, NH_API_UTF32 codepoint)
{
    tk_core_Glyph Glyph;
    memset(&Glyph, 0, sizeof(tk_core_Glyph));
    Glyph.codepoint = codepoint;
 
    tk_core_Glyph *Glyph_p = *Glyphs_pp;
    (*Glyphs_pp)[0] = Glyph;
...
static int tk_core_renderTextFileLineNumber(
    tk_core_Program *Program_p, tk_core_TextFile *TextFile_p, tk_core_Glyph **Glyphs_pp, int line)
{
    char maxX_p[16] = {'\0'};
    ...
    for (int i = 0; i < strlen(xy_p); ++i) {
        tk_core_Glyph *Glyph_p = tk_core_setNextGlyph(Glyphs_pp, xy_p[i]);
        if (xy_p[i] != ' ') {
...
static int tk_core_computeRenderSegmentOffset(
...
static int tk_core_computeRenderSegment(
    ...
    tk_core_Glyph *Glyphs_p, int line)
{
    ...
    {
        TextFile_p->lineNumberOffset = tk_core_renderTextFileLineNumber(Program_p, TextFile_p, &Glyphs_p, line);
        tk_core_TextFileLine *Line_p = TextFile_p->Lines.pp[line];

        int offset = 
            tk_core_computeRenderSegmentOffset(
                &Line_p->RenderCodepoints, FileView_p->TextFile.colOffset);
        int length = 
            tk_core_computeRenderSegment(
                &Line_p->RenderCodepoints, offset, FileView_p->width - TextFile_p->lineNumberOffset);

        for (int i = 0; i < length; ++i) {
            tk_core_setNextGlyph(&Glyphs_p, Line_p->RenderCodepoints.p[i+offset]);
        }
    }
    else {
        char maxX_p[16] = {'\0'};
        sprintf(maxX_p, "%d ~", TextFile_p->fileCursorXTarget);
        tk_core_Glyph *Glyph_p = NULL;
        for (int i = 0; i < strlen(maxX_p); ++i) {
            Glyph_p = tk_core_setNextGlyph(&Glyphs_p, ' ');
        }
        Glyph_p->codepoint = '~';
    }
